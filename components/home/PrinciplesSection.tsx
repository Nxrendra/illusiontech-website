...
export function PrinciplesSection({ content }: { content: IPageContentData }) {
  const isMobile = useIsMobile();
  const principles = content.principles?.length ? content.principles : staticPrinciples;
  ...
  return (
    ...
    >
      <div className="container">
        <SectionHeader
          title={content.homePrinciplesHeading ?? 'Our Guiding Principles'}
          description={
            content.homePrinciplesSubheading ??
            'Our work is driven by a core set of values that ensure excellence, partnership, and innovation in everything we create.'
          }
          isMobile={isMobile}
        />
        <motion.div
          ...
          {principles.map((principle) => {
            ...
            return (
              <motion.div key={principle.title} variants={createItemVariants(isMobile)} whileHover={{ y: -5, scale: 1.02 }} transition={{ type: 'spring', stiffness: 300 }} className="bg-card text-card-foreground p-8 rounded-xl shadow-md text-center transition-shadow hover:shadow-lg">
                <div className="flex items-center justify-center w-16 h-16 mx-auto mb-6 bg-accent/10 rounded-full">{icon && React.cloneElement(icon, { className: 'w-10 h-10 text-accent' })}</div>
