...
if (!EMAIL_SERVER_HOST || !EMAIL_SERVER_PORT || !EMAIL_SERVER_USER || !EMAIL_SERVER_PASSWORD || !EMAIL_FROM) {
    console.warn(
      "Email server environment variables are not fully configured. Email sending will be disabled."
    );
}
...
export async function sendEmail({ to, subject, html, text }: MailOptions) {
  ...
  const options = { from: `"Illusion Tech" <${EMAIL_FROM}>`, to, subject, html, text };
